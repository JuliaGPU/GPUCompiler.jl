...
# All Julia branches are built by the single `julia-ci` pipeline; the per-release
# `julia-release-X-dot-Y` and `julia-master` pipelines were archived in July 2026.
PIPELINE="julia-ci"

# derive the branch to look at from the version
