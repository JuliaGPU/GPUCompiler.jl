...
function irgen(@nospecialize(job::CompilerJob))
    ...
    relocations = collect_julia_value_relocations!(job, mod, gv_to_value)
...
# Compare an LLVM type converted from a Julia type against one read off a kernel signature,
# ignoring pointer address spaces. Post-optimization the two legitimately disagree there:
# `RemoveJuliaAddrspacesPass` strips Julia's tracked/derived address spaces during
# optimization, and a back-end may then move parameters into a target address space (Metal's
# device space, say), while `convert(LLVMType, T; allow_boxed=true)` keeps reporting
# `addrspace(10)`. Everything else about the shape still has to match.
function same_type_modulo_addrspaces(@nospecialize(a::LLVMType), @nospecialize(b::LLVMType))
    a == b && return true
    (a isa LLVM.PointerType && b isa LLVM.PointerType) || return false
    supports_typed_pointers(context()) || return true
    return same_type_modulo_addrspaces(eltype(a), eltype(b))
end
    ...
    for (source_typ, source_name) in zip(source_types, source_argnames)
        ...
        if codegen_typ isa LLVM.PointerType
            ...
            elseif llvm_source_typ isa LLVM.PointerType
                # a boxed argument is `addrspace(10)`-tracked when Julia emits it, but lands
                # in whatever address space the back-end picked once optimization has run
                @assert llvm_source_typ == codegen_typ ||
                        (post_optimization &&
                         same_type_modulo_addrspaces(llvm_source_typ, codegen_typ))
