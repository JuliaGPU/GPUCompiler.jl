...
@testset "replace i128 allocas" begin
    ...
    @test @filecheck begin
        ...
        check"CHECK-LABEL: define void @{{(julia|j)_kernel_[0-9]+}}"
        @static (VERSION >= v"1.12" && VERSION < v"1.12.5") && check"CHECK: alloca <2 x i64>, align 16"
    ...
    @test @filecheck begin
        check"CHECK-NOT: i128"
        check"CHECK-LABEL: define void @{{(julia|j)_kernel_[0-9]+}}"
        @static (VERSION >= v"1.12" && VERSION < v"1.12.5") && check"CHECK: alloca [2 x <2 x i64>], align 16"
